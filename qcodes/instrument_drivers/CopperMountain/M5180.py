...
class FrequencySweepRealImag(MultiParameter):
    """
    Sweep that returns magnitude and phase.
    """
    ...
    def __init__(self,
        ...
        super().__init__(
            ...
            instrument=instrument,
            names=(
                f"{instrument.short_name}_{name}_real",
                f"{instrument.short_name}_{name}_imag"),
            labels=(
                f"{instrument.short_name} {name}",
                f"{instrument.short_name} {name}",
            ),
            units=("dB", "rad"),
            setpoint_units=(("Hz",), ("Hz",)),
...
class M5180(VisaInstrument):
    ...
    def __init__(self, name       : str,
        ...
        self.add_parameter(name='marker1x',
                           label='marker1 x value',
                           get_parser=float,
                           get_cmd='CALC1:SEL:MARK1:X?',
                           unit='Hz')

        self.add_parameter(name='marker1y',
                           label='marker1 y value',
                           get_parser=float,
                           get_cmd=self._get_marker1y,
                           unit='dB')
        ...
        self.add_parameter(name='s21_real_imag',
                           start=self.start(),
                           stop=self.stop(),
                           npts=self.npts(),
                           parameter_class=FrequencySweepRealImag)
    ...
    def _get_marker1y(self) -> None:
        value=self.ask('CALC1:SEL:MARK1:Y?')
        value=value.split(",")
    ...
    def set_marker1_search_min(self) -> None:
        """
        Resets average count to 0
        """
        self.write('CALC1:SEL:MARK1:FUNC:TYPE = "MIN"')

    def set_marker1_search_max(self) -> None:
        """
        Resets average count to 0
        """
        self.write('CALC1:SEL:MARK1:FUNC:TYPE = "MAX"')
